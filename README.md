...
```go
Label(p, "name")               // on soft failure (no input consumed), replace message with name
MapError(p, func(in, err) err) // on any failure, call f to produce a replacement error
